...
function allmonitorsnet() {
	...
	foreach ( $aa as &$value ) {
		if (found ( $value, "mbutton" )) { // look for: style="" border=0
			$non_unique [] = between ( "addtofavorites/monitor/", "\"", $value );
		}
	}
	$counted = array_count_values($non_unique);
	//$unique = $non_unique;
	$unique = array_unique ( $non_unique );
	$sites = array_values ( $unique );
...
// 		$country_rank = ( string ) $xml->SD [1]->attributes ()->RANK;
		...
		echo $web . "," . $rank . "," . $country . "," . $country_rank .",". $counted[$value]."<br/>";
		//echo $web . "," . $rank . ",". $counted[$value]."<br/>";
